// GitHub Models 호출 공용 헬퍼 — ⚠ 이 엔드포인트는 2026-07-30자로 폐지됐다.
//
// 남겨 둔 이유: 휴면 상태인 가이드 백필 스크립트(generate-quest-guides / generate-story-guides)가
// 아직 이걸 import한다. 일일 브리핑·주간 리포트는 AI를 떼고 규칙 기반으로 바꿨다(Phase 45).
// 무료 AI 제공자를 새로 붙이게 되면 이 파일 하나만 갈아끼우면 된다 —
// callModel의 URL·인증 헤더·모델 목록을 바꾸고, 호출부는 그대로 두는 구조.
// 후보 조사 결과는 DESIGN.md Phase 45 참고 (Gemini / Groq 등 OpenAI 호환 무료 티어).
//
// 호출 횟수 로깅 + 상한 안전장치는 그대로 둔다 (버그로 인한 폭주 방지).
...
export async function callModel({ model, system, user, token, purpose }) {
  if (!token) {
    throw new Error(
      'AI 제공자 없음 — GitHub Models는 2026-07-30 폐지됐다 (DESIGN.md Phase 45 참고)',
    )
  }
