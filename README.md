...
실시간 플리마켓 시세·가성비 분석에, **매일 아침 9시 자동 발행되는 일일 브리핑**을 더했습니다.
...
| 탭 | 설명 |
|---|---|
| 오늘의 브리핑 | 패치노트·버그 제보·커뮤니티 동향·신규 영상을 매일 오전 9시(KST) 자동 수집·분류. 이전 날짜·주간 리포트도 조회 가능 |
...
- **브리핑**: GitHub Actions가 매일 EFT 위키 체인지로그·Reddit(인기글+버그 제보)·YouTube 신규 영상·Steam 뉴스를 수집해, 규칙 기반으로 분류(패치/주의/공략/커뮤니티/영상)·중복 제거·신규 표시한 뒤 정적 JSON으로 커밋 — 사람 개입 없이 완전 자동. 매주 월요일엔 주간 리포트도 발행
- 외부 AI 제공자에 의존하지 않아(2026-08 GitHub Models 폐지 이후 규칙 기반으로 전환) 브리핑이 비는 날이 없음
...
**Q. 브리핑에 AI 요약이 없는 이유는?**
A. 원래는 GitHub Models(Actions 기본 `GITHUB_TOKEN`으로 무료 호출)를 썼지만 2026-07-30자로 완전 폐지됐습니다. "모든 것이 무료 + 외부 계정 없음" 원칙을 지키면서 대체할 곳이 마땅치 않아, AI 대신 규칙 기반 큐레이션으로 확정했습니다. 수집기가 붙인 피드 라벨로 섹션을 나누고(버그·PSA → 주의, 공략·팁 → 팁), URL로 중복을 제거하고, 어제 브리핑과 대조해 새 항목만 신규 표시합니다. 대신 영어 원문을 한국어로 옮기는 일은 하지 못해 발췌를 원문 그대로 싣습니다.

**Q. 왜 GitHub Actions cron을 6개나 등록했나?**
A. 운영 중 GitHub의 schedule 이벤트가 이틀 연속(2026-06-12·13) 통째로 누락되는 사고를 겪었기 때문입니다. 정각(혼잡 슬롯) 회피만으론 부족했고, 아침에 몰아둔 cron 3개가 전부 죽는 날도 있어서 09시~20시 하루 전체에 6개를 분산했습니다. 워크플로우 맨 앞의 guard 스텝이 "오늘 브리핑이 이미 있으면 skip"으로 멱등화해 주므로, 백업 cron이 아무리 많아도 중복 생성·재배포 비용은 0입니다.
...
- **일일 브리핑은 자동 수집·분류된 원문 발췌입니다.** 부정확하거나 오래된 정보가 포함될 수 있으며, 정확한 내용은 각 항목의 원문 출처를 확인하세요. 본 사이트의 정보(시세·공략 포함)를 활용한 게임 내 판단과 그 결과는 이용자 본인의 책임입니다.
