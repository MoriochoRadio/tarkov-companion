...
- **schedule 트리거 자체가 신뢰 불가** — 정각 회피로도 부족(2026-06-12·13 연속 누락, schedule 이벤트 0건). 대응은 **cron 다중화**: 한 워크플로우에 시각 분산 cron을 여러 개 등록해 아침에 하나라도 성공하게 함. 단, 생성 스크립트가 매번 덮어쓰므로(멱등 아님) 워크플로우 맨 앞에 "오늘 KST 파일 있으면 이후 단계 skip"하는 guard 스텝 필수. 상세는 DESIGN.md Phase 27
